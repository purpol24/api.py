...
DB_NAME = os.getenv("DB_DATABASE", "gabaykareradb")
    ...
    for skill in skills:
        ...
        if skill_lower in SYNONYMS:
            for synonym in SYNONYMS[skill_lower]:
                expanded.extend(synonym.lower().split())
        else:
            expanded.extend(skill_lower.split())
    return expanded
...
def match_mentors():
    try:
        ...
        # Connect to the MySQL database using env vars
